...
class Item:
    ...
    def copy_sort_order(cls, order=None):
        ...
        result = cls._compare_numeric_desc(
            cls._longest_side(left),
            cls._longest_side(right),
        )
        if result:
            return result
        ...
        result = cls._compare_string_desc(getattr(left, "name", ""), getattr(right, "name", ""))
        if result:
            return result
