(function () {
    function recordPageInit() {
        ...
        window.__pageCleanup.push(function () {
            stopPolling();
            // Restore the global sidebar state handler
            if (typeof window.__applySidebarState === 'function') {
                window.applyCaptureState = window.__applySidebarState;
            }
            window.restartCapture = undefined;
            window.triggerTrafficParticle = undefined;
            window.showCharacterCaptureAnimation = undefined;
        });
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', recordPageInit, { once: true });
    } else {
        recordPageInit();
    }
})();
